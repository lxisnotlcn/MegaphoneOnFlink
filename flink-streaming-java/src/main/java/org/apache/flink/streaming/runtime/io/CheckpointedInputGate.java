...
public class CheckpointedInputGate implements PullingAsyncDataInput<BufferOrEvent>, Closeable {
	...
	// variables for megaphone

	private StreamTask streamTask;

	private int currentControlID = -1;
	...
	private Optional<BufferOrEvent> handleEvent(BufferOrEvent bufferOrEvent) throws IOException, InterruptedException {
		...
		else if (eventClass == InitEvent.class || eventClass == ControlEvent.class || eventClass == CleanEvent.class) {
			if(eventClass == ControlEvent.class){
				if(((ControlEvent)bufferOrEvent.getEvent()).getID() == currentControlID){
					return Optional.of(bufferOrEvent);
				}
				currentControlID = ((ControlEvent)bufferOrEvent.getEvent()).getID();
			}
			this.mailboxExecutor.execute(
				() -> {this.streamTask.handleEvent(bufferOrEvent.getEvent());},
				"handle user event"
			);
		}
